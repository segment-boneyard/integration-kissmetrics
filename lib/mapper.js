...
exports.track = function(track) {
  ...
  var ret = extend(properties, {
    _p: track.userId() || track.sessionId(),
    _t: toUnixTimestamp(track.timestamp()),
    _n: track.event(),
    _d: 1
  });
  if (track.ip()) ret._ip = track.ip();
  if (track.userAgent()) ret._ua = track.userAgent();
  return ret;
};
...
exports.page = function(page) {
  ...
  if (page.ip()) ret._ip = page.ip();
  if (page.userAgent()) ret._ua = page.userAgent();
  return ret;
};
...
exports.screen = function(screen) {
  ...
  if (screen.ip()) ret._ip = screen.ip();
  if (screen.userAgent()) ret._ua = screen.userAgent();
  return ret;
};
